...
final class OnlineDistrictTileOverlay: MKTileOverlay {
    ...
    init(source: OnlineDistrictMap, tileLoader: TileLoader? = nil) {
        ...
        self.continuity = RasterMapContinuity(bounds: source.bounds,
    ...
    override var boundingMapRect: MKMapRect { source.bounds }
    override var coordinate: CLLocationCoordinate2D {
        MKMapPoint(x: source.bounds.midX, y: source.bounds.midY).coordinate
    }
    ...
    override func loadTile(at path: MKTileOverlayPath, result: @escaping (Data?, Error?) -> Void) {
        guard source.containsTile(path) else {
            result(nil, nil)
            return
        }
        tileLoader(url(forTilePath: path), source.cacheKey(for: path), result)
    }
}
