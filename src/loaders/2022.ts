...
export async function load2022(): Promise<YearBatch> {
  const year = 2022;

  const opsPath = path.resolve(process.cwd(), "src/data/2022/ops_2022.csv");
  const riversPath = path.resolve(process.cwd(), "src/data/2022/rivers_2022.csv");
  const regPath = path.resolve(process.cwd(), "src/data/2022/registration_2022.csv");
  ...
  return {
    meta: {
      year,
      allocationEndDate: "2022-08-03",
      // Required by YearMeta (confirm later from report narrative)
      togiakOutsiderOpenDate: "2022-07-27",
