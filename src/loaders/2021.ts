...
export async function load2021(): Promise<YearBatch> {
  const year = 2021;

  const opsPath = path.resolve(process.cwd(), "src/data/2021/ops_2021.csv");
  const riversPath = path.resolve(process.cwd(), "src/data/2021/rivers_2021.csv");
  const regPath = path.resolve(process.cwd(), "src/data/2021/registration_2021.csv");
  ...
  return {
    meta: {
      year,
      allocationEndDate: "2021-08-03",
      // confirm later from narrative
      togiakOutsiderOpenDate: "2021-07-27",
